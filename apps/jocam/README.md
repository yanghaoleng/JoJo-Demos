...
- 叫叫按 Rive 画布左下角锚定，高度上限为原尺寸的 51.2%；每次切换动作都会自动检测透明边界，完整显示后将最左可见内容贴齐取景框。以 `Start_Dial` 开始，每秒从筛选后的 `TalkingEmotion` 时间轴中随机切换。
...
- 手机竖屏时，成片取景框避开刘海与系统安全区；人像图层、前后摄像头和快门位于独立的纯黑操作区，不会进入照片或录像。
