...
`/report-codex/` 为独立重建版本，演示每日、每周日晚、每月和阶段四类家长触达。日报按“产品触达 / 班班触达”分别双图并排，阶段报告同时展示封面与完整报告；日报、月报、阶段报均在固定高度窗口内纵向滚动。图片随首屏统一预加载，课程预告直接载入独立 HTML，插画弱化为右侧水印；静态截图统一使用 WebP。无 API、无用户数据采集。
